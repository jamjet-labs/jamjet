...
/// The resource every exporter stamps on its telemetry.
///
/// Built per exporter rather than shared: `Resource` is cheap, and threading one
/// value through two builders that are constructed at different times invites the
/// tracer and the meter disagreeing about what service produced the data.
...
fn build_otlp_tracer(endpoint: &str) -> Result<opentelemetry_sdk::trace::Tracer, String> {
    ...
    let tracer = provider.tracer("jamjet");
    // The provider must outlive the tracer, and the global is what flushes the
    // batch on shutdown. Dropping it here would leave a tracer whose exports go
    // nowhere — silently, which is the failure mode telemetry can least afford.
